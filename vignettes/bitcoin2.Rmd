...
## D-Vine Copula of second kind

We fit a full model combining the double Weibull margin with the VT-d-vine model.

```{r,fig.show='hold', dev.args =list(pointsize=9)}
fullmod2 <- tscm(fit_Frank, margin = marg_dw)
fullmod2 <- fit(fullmod2, as.numeric(X), 
               method = "full", tsoptions = tsoptions)
fullmod2
AIC(marg_dw, fullmod, fullmod2)
```

```{r,fig.show='hold', dev.args =list(pointsize=9)}
plot(fullmod2, plotoption = 1)
plot(fullmod2, plotoption = 2)
plot(fullmod2, plottype = "margin")
plot(fullmod2, plottype = "vtransform")
plot(fullmod2, plottype = "volprofile")
plot(fullmod2, plottype = "volproxy")
plot(fullmod2, plotoption = 2, plottype = "volproxy")
```
